...
- `admin.html` uses a mobile-app-style shell inspired by the Jordanian "SANAD" government app: fixed top bar + fixed bottom tab bar (`#adm-bottomnav`, dash/qs/centers/more) + a "more" bottom sheet (`#adm-more-sheet`) for the remaining sections (users/reviews/locations/logout), instead of the old desktop sidebar. This applies at all screen widths, not just mobile — the user explicitly chose full mobile-app style over a desktop-sidebar-with-reskin approach.
