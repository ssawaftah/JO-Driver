...
import type { FooterSponsor } from "../types";

const DEFAULT_SPONSOR_IMG = "/sponsor-default.png";

const SOCIAL_SVGS: Record<string, { label: string; svg: string; bg: string }> = {
  facebook: {
    label: "Facebook",
    bg: "#1877F2",
  ...
  x: {
    label: "X",
    bg: "#000000",
    svg: `<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`,
  },
};

const DEFAULT_ABOUT =
  "منصة JO Driver هي دليلك الأول لاجتياز امتحان القيادة النظري في الأردن. نقدم لك أسئلة تدريبية شاملة، دليل عملي، وقائمة بأفضل مراكز التدريب المعتمدة.";
...
export default function AppFooter() {
  ...
  useEffect(() => {
    ...
    ]).then(([spSnap, soSnap, dlSnap, atSnap]) => {
    ...
    <footer style={{
      ...
      <div style={{ padding: "28px 16px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 900, color: "#f8fafc" }}>JO Driver</div>
        </div>
        <p style={{
          fontSize: 13, color: "#94a3b8", lineHeight: 1.8, margin: 0,
          maxWidth: 420,
        }}>
          {aboutText}
        </p>
      ...
      <div style={{ padding: "0 16px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
          <span style={{ fontSize: 10, fontWeight: 800, color: "#64748b", letterSpacing: 0.5, textTransform: "uppercase" }}>الراعي الرسمي</span>
          <div style={{ flex: 1, height: 1, background: "#1e293b" }} />
          {totalSlides > 1 && (
            <span style={{ fontSize: 10, color: "#475569" }}>{current + 1}/{totalSlides}</span>
          )}
        </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 5, marginTop: 10 }}>
            {displaySponsors.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} style={{
                ...
                background: i === current ? "#246BFD" : "#334155",
                transition: "all 0.3s",
              }} />
            ))}
          </div>
      ...
      {activeSocials.length > 0 && (
        <div style={{ padding: "12px 16px", borderTop: "1px solid #1e293b" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 10, fontWeight: 800, color: "#64748b" }}>تواصل معنا</span>
            <div style={{ flex: 1, height: 1, background: "#1e293b" }} />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {activeSocials.map(([key, { svg, label, bg }]) => (
      ...
      {/* ── Copyright ── */}
      <div style={{
        textAlign: "center", padding: "12px 16px 16px",
