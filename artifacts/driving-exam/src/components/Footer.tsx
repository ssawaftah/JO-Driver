...
const DEFAULT_SPONSOR_IMG = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(`
<svg width="700" height="160" viewBox="0 0 700 160" xmlns="http://www.w3.org/2000/svg">
  <rect width="700" height="160" rx="12" fill="#EEF4FF"/>
  <rect x="1" y="1" width="698" height="158" rx="11" fill="none" stroke="#246BFD" stroke-width="2" stroke-dasharray="8 6"/>
  <text x="350" y="62" font-family="Tajawal,Arial" font-size="22" font-weight="900" fill="#246BFD" text-anchor="middle">🌟 كن الراعي الرسمي لـ JO Driver</text>
  <text x="350" y="95" font-family="Tajawal,Arial" font-size="15" fill="#6B7280" text-anchor="middle">تواصل معنا لعرض شعارك هنا</text>
  <text x="350" y="125" font-family="Tajawal,Arial" font-size="13" fill="#9CA3AF" text-anchor="middle">اضغط للتواصل</text>
</svg>`);
...
const SOCIAL_ICONS: Record<string, { svg: string; label: string; color: string }> = {
  ...
  x: {
    label: "تويتر / X",
    color: "#000000",
    svg: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`,
  },
};
...
export default function AppFooter() {
  ...
  const displaySponsors = sponsors.length === 0 ? [{ id: "default", imageUrl: DEFAULT_SPONSOR_IMG, link: "" }] : sponsors;
    ...
    <div style={{ background: "#F9FAFB", borderTop: "1px solid #E5E7EB", direction: "rtl" }}>
      ...
      <div style={{ padding: "16px 16px 12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
          <span style={{ fontSize: 11, fontWeight: 800, color: "#9CA3AF", letterSpacing: 0.5, textTransform: "uppercase" }}>الراعي الرسمي</span>
              ...
              <div key={sp.id} style={{ width: `${100 / totalSlides}%`, flexShrink: 0, padding: "0 1px" }}>
                <img
                  ...
                  style={{
                    ...
                    display: "block", cursor: sp.link ? "pointer" : "default",
      ...
      {activeSocials.length > 0 && (
        <div style={{ padding: "10px 16px 12px", borderTop: "1px solid #F3F4F6" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 12 }}>
            {activeSocials.map(([key, { svg, label, color }]) => (
              <a key={key} href={(social as any)[key]} target="_blank" rel="noopener noreferrer"
                style={{
                  width: 40, height: 40, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center",
                  background: "#fff", border: "1.5px solid #E5E7EB", color, flexShrink: 0,
                  textDecoration: "none",
                }}
                title={label}>
